...
export const RoadmapGridCard: React.FC<RoadmapGridCardProps> = ({ 
    ...
    // Check if any bites are available to show completion status or prompt
    const hasBites = dailyBites && Object.keys(dailyBites).length > 0;
        ...
        <div className="col-span-12 w-full">
            ...
            `}>
                ...
                <div className="relative flex flex-col lg:flex-row lg:min-h-[440px] px-0 lg:px-2 py-4">
                    ...
                    {/* Lateral Navigation (Internal Hub) - Desktop */}
                    ...
                    {/* Mobile Tab Switcher */}
                    <div className="flex lg:hidden w-full absolute top-4 left-0 right-0 justify-center z-30 px-6 gap-4">
                        {[
                            { id: 'path', label: 'Path' },
                            ...
                            <button
                                key={item.id}
                                onClick={() => setActiveSubTab(item.id as any)}
                                className={`flex-1 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 border ${
