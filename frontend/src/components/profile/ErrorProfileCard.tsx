...
export const ErrorProfileCard: React.FC<ErrorProfileCardProps> = ({ data, className = "" }) => {
    return (
        <MissionControlCard className={`${className} !p-0`} title="Linguistic Error Model">
            <div className="p-8 pt-4">

                <div className="h-[350px] w-full flex items-center justify-center relative min-w-0">
                    {data && data.length > 0 ? (
                        <ResponsiveContainer width="100%" aspect={1.2} debounce={200}>
                    ...
                    {data.slice(0, 3).map((w, i) => (
                        <div key={i} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-white/5 rounded-2xl border border-slate-100 dark:border-white/5">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-[11px] font-bold text-slate-800 dark:text-white/80">{w.subject}</span>
                            </div>
                            <span className="text-[10px] font-black text-rose-500 bg-rose-500/10 px-2 py-0.5 rounded uppercase tracking-widest">{w.A}% Density</span>
                        </div>
                    ))}
                </div>
            </div>
