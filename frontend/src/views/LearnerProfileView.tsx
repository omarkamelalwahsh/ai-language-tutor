  ...
  <GlassCard className="flex flex-col gap-6">
    <div className="flex justify-between items-start">
      ...
      <div className="text-right">
        <span className={`text-[10px] uppercase font-black tracking-widest px-2 py-1 rounded-full border ${
          skill.stability === 'Stable' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-amber-500/10 border-amber-500/20 text-amber-400'
        }`}>
          {skill.stability}
        </span>
      </div>
...
const ErrorCard = ({ error }: { error: any }) => (
    <div className="flex items-center justify-between p-6 bg-white/[0.03] border border-white/5 rounded-3xl hover:bg-white/[0.06] transition-all group/err">
        <div className="flex items-center gap-4">
            <div className={`p-3 rounded-2xl ${
                error.severity === 'High' ? 'bg-rose-500/10 text-rose-400' : 'bg-amber-500/10 text-amber-400'
            }`}>
                <AlertCircle size={20} />
            </div>
            <div>
      ...
      <div className="max-w-7xl mx-auto px-6 pt-12">
        ...
        <header className="flex items-center justify-between mb-16">
            ...
            <div className="flex items-center gap-6">
                ...
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-500 to-indigo-700 p-[2px]">
            ...
            >
                <GlassCard className="lg:col-span-2 flex flex-col md:flex-row gap-8 items-center bg-blue-600 dark:bg-blue-600/5">
                    <div className="relative flex-shrink-0">
                        ...
                        <div className="absolute -bottom-2 -right-2 bg-indigo-500 p-2 rounded-xl shadow-lg">
            ...
            <section className="space-y-8">
                <GlassCard>
                    <h3 className="text-xl font-black mb-6 flex items-center gap-3">
                        <Calendar size={20} className="text-indigo-400" /> Retention Model Queue
                    </h3>
                ...
                <GlassCard>
                    ...
                    <div className="flex items-center gap-8 mb-6">
                         <div className="relative w-24 h-24 flex items-center justify-center">
                            ...
                            <Gauge size={24} className="absolute text-amber-500" />
                        </div>
                        <div>
            ...
            <div className="max-w-xl w-full bg-white dark:bg-gray-900 rounded-[2.5rem] p-10 shadow-sm dark:shadow-md shadow-slate-200/50 border border-slate-200 dark:border-gray-800 flex flex-col min-h-[600px] relative overflow-hidden">
                ...
                <div className="relative z-10 flex items-center gap-6">
                    <div className="w-14 h-14 bg-black rounded-2xl flex items-center justify-center animate-pulse">
                        <Sparkles className="text-indigo-400" fill="currentColor" />
                    </div>
                    <div>
                        <p className="text-[10px] font-black uppercase tracking-[0.25em] text-black/40 mb-1">Synthesized Recommendation</p>
                        <h4 className="text-xl font-black tracking-tight">{data.best_next_move}</h4>
                    </div>
                </div>

                <button 
                    onClick={() => navigate('/runtime')}
                    className="relative z-10 px-8 py-4 bg-black text-white rounded-2xl font-black uppercase tracking-widest text-xs hover:scale-105 active:scale-95 transition flex items-center gap-3 group/btn"
                >
                    Start Now <ArrowRight size={14} className="group-hover/btn:translate-x-1 transition-transform" />
                </button>
            </div>
        </motion.div>
      </div>
