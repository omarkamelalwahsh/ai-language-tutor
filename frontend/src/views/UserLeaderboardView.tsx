...
export const UserLeaderboardView: React.FC<UserLeaderboardViewProps> = ({ onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-gray-950 p-6 md:p-10 font-sans text-slate-900 dark:text-slate-50 selection:bg-indigo-500/30 transition-colors duration-300">
      <div className="max-w-4xl mx-auto">
        <header className="flex items-center justify-between mb-8">
          <button onClick={onBack} className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-xl text-slate-500 dark:text-slate-400 hover:bg-white dark:bg-gray-900-hover transition-colors font-bold text-sm">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl font-extrabold tracking-tight text-slate-900 dark:text-slate-50 mb-3">Global Leaderboard</h1>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-[2rem] shadow-sm dark:shadow-md shadow-slate-200/50 border border-slate-200 dark:border-gray-800 overflow-hidden relative">
                  ...
                  return (
                    <motion.div 
                      ...
                      className={`flex items-center p-4 sm:p-5 rounded-2xl transition-all ${
                        isCurrentUser ? 'bg-blue-50 dark:bg-blue-900/30 border-2 border-indigo-500 shadow-md shadow-indigo-100' : 'bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 hover:border-foreground/20'
                      }`}
                    >
                      ...
                      <div className="flex-1 min-w-0 px-4">
                        <div className="flex items-center gap-2">
                          <h3 className={`font-bold truncate ${isCurrentUser ? 'text-indigo-900 text-lg' : 'text-slate-800 dark:text-slate-200'}`}>
                            {entry.displayName}
                          </h3>
                          {isCurrentUser && <span className="bg-blue-600 dark:bg-blue-600 text-white text-[10px] font-black uppercase px-2 py-0.5 rounded-full tracking-widest">You</span>}
                        </div>
                      ...
                      <div className="text-right shrink-0">
                        <div className="text-xl sm:text-2xl font-black text-slate-900 dark:text-slate-50 tracking-tight">{entry.score.toLocaleString()} <span className="text-sm font-bold text-slate-400 uppercase tracking-widest">XP</span></div>
